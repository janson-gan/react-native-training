...
### Mistake I Make
During the implementation of control flow, I created two if-else statement to check for the state is null to perform incrementation and decrementation of rgb color. I was so happy that I solve the 'null' problem and forgotton that two function is only meant for the red button! What about the green and the blue? Including red button means I need to create Six functions to handle the increment and decrement.😬 Fortunately, there's one way to minimise these hassels. Maybe I can use ternary operator inside the button itself rather than seperate them and occupy so much space. Let's do it.
